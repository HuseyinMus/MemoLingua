...
import { generateAudio, playGeminiAudio } from '../services/geminiService';
    ...
    <div className="fixed inset-0 z-[9999] flex items-center justify-center bg-black/90 backdrop-blur-md p-6 animate-fade-in">
        <div className="bg-white dark:bg-zinc-900 w-full max-w-sm rounded-[2.5rem] p-8 text-center shadow-2xl border border-zinc-200 dark:border-zinc-800 animate-slide-up">
            ...
            <div className="space-y-3">
                <button onClick={() => {
                    onRestart();
                    if (Math.random() > 0.4) adService.showInterstitial(); // Show ad on restart 40% of the time
                }} className="w-full py-4 bg-black dark:bg-white text-white dark:text-black font-bold rounded-xl shadow-lg hover:scale-[1.02] active:scale-95 transition-all flex items-center justify-center gap-2">
                    <RotateCcw size={18} /> Tekrar Oyna
                </button>
                <button onClick={() => {
                    onExit();
                    adService.showInterstitial(); // Show ad on exit 100% of the time (high revenue)
                }} className="w-full py-4 bg-zinc-100 dark:bg-zinc-800 text-black dark:text-white font-bold rounded-xl hover:bg-zinc-200 dark:hover:bg-zinc-700 transition-colors">
                    Arena Menüsüne Dön
                </button>
            </div>
        </div>
...
const useHangman = (words: UserWord[], onEnd: (score: number, isWin: boolean) => void) => {
    ...
    const guess = (char: string) => {
        ...
        if (!word.term.toLowerCase().includes(lowerChar)) {
            ...
            const isWon = word.term.split('').every(c =>
                !/[a-zA-ZçğışöüÇĞİŞÖÜ]/.test(c) || newGuessed.has(c.toLowerCase())
            );
            if (isWon) {
                setStatus('won');
                onEnd(50, true);
...
const useScramble = (words: UserWord[], onEnd: (score: number, isWin: boolean) => void) => {
    ...
    const nextWord = useCallback(() => {
        ...
        // Sadece harfleri karıştır, boşlukları ve özel karakterleri koru
        // Boşlukları ve harfleri ayır, ancak sadece harfleri karıştır
        const cleanTerm = random.term.trim();
        const chars = cleanTerm.toUpperCase().split('').filter((c: string) => /[A-ZÇĞİŞÖÜ]/.test(c)).map((c: string, i: number) => ({
            id: `tile-${i}-${c}-${Math.random()}`,
            char: c,
    ...
    useEffect(() => {
        if (currentWord && placedTiles.length === currentWord.term.toUpperCase().replace(/[^A-ZÇĞİŞÖÜ]/g, '').length) {
            const attempt = placedTiles.map(t => t.char).join('');
            const correctAnswer = currentWord.term.toUpperCase().replace(/[^A-ZÇĞİŞÖÜ]/g, '');
...
const useAudioQuiz = (words: UserWord[], onEnd: (score: number, isWin: boolean) => void) => {
    ...
    const generateQuestion = useCallback(() => {
        ...
        setCurrentWord(target);
        setOptions(allOptions);
        // Play audio with a slight delay
...
export const Games: React.FC<GamesProps> = ({ userProfile, words, onAddXP, leaderboardData }) => {
        ...
        return (
            <div className="flex flex-col items-center justify-center h-full p-6 animate-fade-in relative overflow-hidden">
                ...
                <div className="mb-8 relative w-full flex justify-center">
                    <svg viewBox="0 0 120 160" className="w-32 sm:w-40 stroke-black dark:stroke-white stroke-2 fill-none drop-shadow-sm">
                        <line x1="10" y1="150" x2="110" y2="150" />
                        <line x1="60" y1="150" x2="60" y2="20" />
                        <line x1="60" y1="20" x2="100" y2="20" />
                        <line x1="100" y1="20" x2="100" y2="40" />
                        {lives <= 5 && <circle cx="100" cy="50" r="10" className="animate-pulse" />}
                ...
                <div className="text-3xl font-mono tracking-[0.3em] font-bold mb-6 text-center break-words flex flex-wrap justify-center gap-x-2 gap-y-4 px-4 w-full">
                    {word?.term.split('').map((char: string, i: number) => {
                        const isNonAlpha = !/[a-zA-ZçğışöüÇĞİŞÖÜ]/.test(char);
                        return (
                            <span key={i} className={`border-b-2 border-zinc-300 dark:border-zinc-700 inline-block min-w-[1.5rem] h-10 text-black dark:text-white ${char === ' ' ? 'border-none w-4' : ''}`}>
                                {guessed.has(char.toLowerCase()) || isNonAlpha || status !== 'playing' ? char : ''}
                            </span>
                        );
                    })}
                </div>
                ...
                {status === 'playing' && (
                    <div className="flex flex-wrap justify-center gap-1.5 max-w-sm">
                        {'abcçdefgğhıijklmnoöprsştuüvwxyz'.split('').map(char => (
                            <button
                                key={char}
                                onClick={() => guess(char)}
                                disabled={guessed.has(char)}
                                className={`w-9 h-10 rounded-lg font-bold text-sm transition-all ${guessed.has(char)
                                    ? 'bg-zinc-100 dark:bg-zinc-800 text-zinc-300 dark:text-zinc-600'
                                    : 'bg-white dark:bg-zinc-700 text-black dark:text-white shadow-sm hover:scale-110 active:scale-95 border-b-4 border-zinc-200 dark:border-zinc-800 active:border-b-0 active:translate-y-1'
                                    }`}
                            >
                                {char.toUpperCase()}
                            </button>
    ...
    const SnakeGame = () => {
        ...
        return (
            <div className="flex flex-col items-center justify-center h-full p-6 animate-fade-in overflow-hidden relative">
                ...
                <div className="relative bg-zinc-200 dark:bg-zinc-800 rounded-3xl border-4 border-zinc-300 dark:border-zinc-700 w-full max-w-[320px] aspect-square shadow-inner overflow-hidden">
                    {isGameOver && (
                        <GameOverModal
                            isWin={score > 30}
                    ...
                    {snake.map((seg, i) => (
                        <div
                            key={i}
                            className={`absolute rounded-sm z-10 transition-all duration-150 ${i === 0 ? 'bg-black dark:bg-white shadow-lg z-20 scale-110' : 'bg-zinc-600 dark:bg-zinc-400 opacity-80'}`}
                            style={{
                                left: `${seg.x * 5}%`,
                                top: `${seg.y * 5}%`,
                                width: '5%',
                    ...
                    <div
                        className="absolute bg-green-500 rounded-full animate-pulse z-0 flex items-center justify-center shadow-lg shadow-green-500/50"
                        style={{
                            left: `${food.x * 5}%`,
                            top: `${food.y * 5}%`,
                            width: '5%',
                ...
                <div className="grid grid-cols-3 gap-3 w-full max-w-sm flex-1 overflow-y-auto content-start py-4">
                    {cards.map((card, index) => (
                        <button
                            ...
                            className={`aspect-[3/4] rounded-xl flex flex-col items-center justify-center p-2 text-center text-xs font-bold shadow-lg transition-all duration-500 transform perspective-1000 ${card.isFlipped || card.isMatched
            ...
            <div className="flex flex-col items-center justify-center h-full p-6 animate-fade-in relative max-w-sm mx-auto overflow-hidden">
                ...
                <div className="w-full text-center relative transition-all duration-300 flex-1">
                    ...
                    <div className="mt-12">
                        ...
                        <div className="flex flex-wrap justify-center gap-2 mb-8 min-h-[3.5rem] w-full">
                            {placedTiles.map((tile) => (
                                <button
                                    key={tile.id}
                                    onClick={() => handleTileClick(tile)}
                                    className="w-9 h-11 sm:w-10 sm:h-12 bg-black dark:bg-white text-white dark:text-black rounded-xl font-black text-xl shadow-lg animate-slide-up flex items-center justify-center border-b-4 border-zinc-700 dark:border-zinc-200 active:border-b-0 active:translate-y-1"
                                >
                                    {tile.char}
                                </button>
                            ))}
                            {Array.from({ length: Math.max(0, (currentWord?.term.toUpperCase().replace(/[^A-ZÇĞİŞÖÜ]/g, '').length || 0) - placedTiles.length) }).map((_, i) => (
                                <div key={i} className="w-9 h-11 sm:w-10 sm:h-12 bg-zinc-200 dark:bg-zinc-800 rounded-xl border-2 border-dashed border-zinc-300 dark:border-zinc-700"></div>
                            ))}
                        </div>
    ...
    if (activeTab === 'leaderboard') {
        // Mock data if no leaderboard provided
        const mockLeaderboard: LeaderboardEntry[] = leaderboardData && leaderboardData.length > 0 ? leaderboardData : [
            { id: '1', rank: 1, name: userProfile?.username || 'Sen', avatar: '👑', xp: userProfile?.xp || 0, isCurrentUser: true },
            { id: '2', rank: 2, name: 'Ali Yılmaz', avatar: '🔥', xp: Math.max(0, (userProfile?.xp || 0) - 50), isCurrentUser: false },
            { id: '3', rank: 3, name: 'Ayşe Demir', avatar: '⭐', xp: Math.max(0, (userProfile?.xp || 0) - 120), isCurrentUser: false },
            { id: '4', rank: 4, name: 'Mehmet Kaya', avatar: '💪', xp: Math.max(0, (userProfile?.xp || 0) - 200), isCurrentUser: false },
            { id: '5', rank: 5, name: 'Zeynep Şahin', avatar: '🎯', xp: Math.max(0, (userProfile?.xp || 0) - 280), isCurrentUser: false },
        ];
