...
import { Volume2, Sparkles, BookOpen, Lightbulb, Mic, Check, AlertCircle, Loader2, ImageIcon, Waves, X, Pencil, MessageCircle, Brain, Activity, ChevronsUpDown } from 'lucide-react';
...
export const StudyCard: React.FC<StudyCardProps> = ({ word, mode, onResult, nextIntervals, autoPlayAudio = true }) => {
  const [currentMode, setCurrentMode] = useState<StudyMode>(mode);
  ...
  const toggleMode = () => {
    const modes: StudyMode[] = ['meaning', 'writing', 'context'];
  ...
  return (
    <div className="relative w-full max-w-[320px] aspect-[3/4.6] mx-auto flex flex-col bg-white dark:bg-zinc-900 rounded-[2.5rem] shadow-[0_32px_64px_-16px_rgba(0,0,0,0.15)] dark:shadow-black/50 border border-zinc-100 dark:border-zinc-800 overflow-hidden transition-all duration-500">
       ...
       <div className="absolute top-0 left-0 right-0 p-6 flex justify-between items-start z-10 pointer-events-none">
           <button 
               onClick={toggleMode}
               className="flex items-center gap-2 px-3 py-1.5 bg-zinc-100 dark:bg-zinc-800 rounded-full pointer-events-auto hover:bg-zinc-200 dark:hover:bg-zinc-700 active:scale-95 transition-all group shadow-sm border border-transparent hover:border-zinc-300 dark:hover:border-zinc-600 cursor-pointer"
           >
               {currentMode === 'meaning' && <Sparkles size={10} className="text-indigo-500" />}
               {currentMode === 'writing' && <Pencil size={10} className="text-blue-500" />}
               {currentMode === 'context' && <MessageCircle size={10} className="text-emerald-500" />}
               
               <span className="text-[9px] font-black uppercase tracking-widest text-zinc-500 group-hover:text-zinc-800 dark:group-hover:text-zinc-200">
                   {currentMode === 'meaning' ? (word.type || 'KART') : currentMode}
               </span>
               <ChevronsUpDown size={10} className="text-zinc-400 group-hover:text-zinc-600 dark:group-hover:text-zinc-300" />
           </button>
           ...
           <div className="flex flex-col items-end gap-2 pointer-events-auto">
               ...
               <button 
                    ...
                    className={`p-2 rounded-full border transition-all active:scale-90 ${visualMnemonic ? 'bg-indigo-600 border-indigo-600 text-white' : 'bg-white dark:bg-zinc-800 border-zinc-200 dark:border-zinc-700 text-zinc-400'}`}
                >
       ...
       {/* Front Face */}
       <div className={`flex-1 flex flex-col p-6 pt-24 ${showAnswer ? 'opacity-0 scale-95 pointer-events-none' : 'opacity-100 scale-100'} transition-all duration-300`}>
           
           <div className="flex-1 flex flex-col items-center justify-center text-center">
                {currentMode === 'meaning' || currentMode === 'writing' ? (
                    <>
           ...
           >
                {currentMode === 'meaning' ? 'Cevabı Gör' : 'Kontrol Et & Gör'}
           </button>
       </div>
