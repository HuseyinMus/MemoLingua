    ...
    const handleAuth = async (e: React.FormEvent) => {
        ...
        try {
            if (isLogin) {
                ...
                // Login successful, App.tsx will handle the state change
                onLoginSuccess();
            } else {
                ...
                // We wrap this in a try-catch because Firestore might be disabled on the backend
                // or the user might be offline. We don't want to block the auth flow.
                try {
                    ...
                    } catch(err) { console.warn("Leaderboard init failed (non-critical)", err); }
                } catch (firestoreError) {
                    console.warn("Firestore initialization failed. Proceeding with Auth only.", firestoreError);
